### Various tree plotting functions
...
##The following functions depend on the PDX data structure
...
add_vaf=function(pdx,##<< PDX object or list including details matrix
  ...
  if(length(samples)>1){
    if(length(info$idx)>1){
      df=data.frame(mtr=rowSums(pdx$mtr[info$idx,samples],na.rm = TRUE),
                    dep=rowSums(pdx$dep[info$idx,samples],na.rm = TRUE),
  ...
  if(!is.null(pdx$details$is_localx_excluded)){
    
    flag=pdx$details$is_localx_excluded[info$idx]
    df$barcol=ifelse(flag==0,barcol,"pink")
  }else{
    df$barcol=barcol
  }
  ...
  for(i in 1:N){
    ...
    if(is.na(vaf)){
      ...
      plotF(x=info$x,y = y[i],radius = r,col=c(df$barcol[i],"white"),prop = c(vaf,1-vaf),width = width)
    }
  }
  for(i in 1:N){
...
plotBars=function(x,y,radius,col,prop,border="black",width=1){
  ...
  if(FALSE){#(width<diff(par("usr")[c(2,4)])/200){
...
add_vertical_labels=function(
  ...
  if(dim(vars)[1]==0){
    if(FALSE){
    if(control$b.extend.colour.down.tree){
      nodes=setdiff(get_parents(thisnode,tree$edge),thisnode)
      idx=match(nodes,control$vars$node)
      idx=idx[which(!is.na(idx))]
      if(length(idx)>0){
        idx=idx[1]# take first index as the most recent ancestor and get colour from there..
        var=control$vars %>% filter(node==nodes[idx])##  could filter only chose one representative colour to sweep down..
  ...
  if(control$b.show.just.group){
    ...
    col=col[match(vlabels,vars$group)]
  }
  N=length(vlabels)
...
plot_basic_tree=function(pdx,label,style="classic",cex.annot.label=1,legpos="topleft",genes=NULL,cv=c(),seed=-1,b.annot.just.shared=FALSE,col.label="black",b.show.just.group=FALSE,b.extend.colours=FALSE,prioritise.group=NULL,...){
  if(b.extend.colours){
    muts=get_all_tree_drivers(pdx,cv = cv,genes=genes)
    muts$child_count=sapply(muts$node,function(node) length(get_samples_in_clade(node,pdx$tree_ml)))
    muts=muts[order(muts$child_count,decreasing = TRUE),]
    #print(muts)
    scheme=get_consequence_scheme(allowed.value=cv)
    colnames(scheme)[1]="cv"
    vars=muts %>% left_join(scheme,by="cv")
    scheme=get_driver_scheme2()
    vars$group=gsub(":.+","",vars$label2)
    vars$col=scheme$colour[match(vars$group,scheme$group)]
    if(dim(vars)[1]>0){
      if(!is.null(prioritise.group)){
        vars$priority=0
        idx=which(vars$group %in% prioritise.group)
        if(length(idx)>0){
          vars$priority[idx]=1
          vars$oidx=match(vars$node,vars$node)
          vars=vars[order(vars$oidx,vars$priority),]
        }
      }
      tmp=markup_tree(pdx$tree_ml,vars$node)
      pdx$tree_ml$color=rep("grey",length(tmp$type))
      idx=which(tmp$type>0)
      if(length(idx)>0)
      {
        pdx$tree_ml$color[idx]=vars$col[tmp$type[idx]]
      }
...
set_color_by_age=function(pdx){
  ...
  age.df$color=c("grey70","grey30",RColorBrewer::brewer.pal(5,"Set1"))[1:dim(age.df)[1]]
  #age.df$color=RColorBrewer::brewer.pal(5,"YlOrBr")[3+(1:dim(age.df)[1])]
...
add_heatmap=function(tree,heatmap,heatvals=NULL,border="black",cex.label=2,pos=4,force.count=-1,txtcols=rep("black",dim(heatmap)[1]),b.add.lines=FALSE,font=1,text.darken.amount=0.3){
  ymax=tree$ymax
  ...
  for(i in 1:dim(heatmap)[1]){
    ...
    if(!is.null(labels)){
      if(get_luminance(txtcols[i])>0.7){
        ## Darken those where labels will be difficult to read
        text(labels[i],x=par("usr")[1]+0.1,y=0.5*(top+bot),pos = pos,cex = cex.label,col=darken(txtcols[i],amount=text.darken.amount),font=font)
      }else{
        text(labels[i],x=par("usr")[1]+0.1,y=0.5*(top+bot),pos = pos,cex = cex.label,col=txtcols[i],font=font)
      }
    }
    top=bot
  }
